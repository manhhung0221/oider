...
def draw_oi(data,vn30,reTicker):
    ...
    fig.add_trace(go.Scatter(x=vn30['DateTime'], y=vn30['Close'], name='VN30',line=dict(dash='dash')), secondary_y=True)
    fig.add_trace(go.Scatter(x=data['DateTime'], y=data[reTicker], name=reTicker,line=dict(dash='dash')), secondary_y=True)
...
def get_data_ticker(Ticker,min_dateX):
    '''min_dateX: Ngày bắt đầu của fig_reTicker'''
    data_td=pd.read_sql(f"""SELECT re.Ticker AS Ticker,re.DateTime,BuyVolTD,SellVolTD 
                    FROM re_ticker AS re
                    INNER JOIN eod_td_derivative AS td
                    ON re.Ticker=td.Ticker AND re.DateTime=td.DateTime
                    WHERE re.Ticker='{Ticker}' AND re.DateTime>='{min_dateX:%Y-%m-%d}'""",engine_public)
    data_nn=pd.read_sql(f"""SELECT re.Ticker AS Ticker,re.DateTime,BuyVolForeign,SellVolForeign
                    FROM re_ticker AS re
                    INNER JOIN eod_nn_derivative AS nn
                    ON re.Ticker=nn.Ticker AND re.DateTime=nn.DateTime
                    WHERE re.Ticker='{Ticker}' AND re.DateTime>='{min_dateX:%Y-%m-%d}'""",engine_public)
    data_eod=pd.read_sql(f"""SELECT  re.Ticker AS Ticker,der.Close AS {Ticker},re.DateTime,der.OI 
                    FROM re_ticker AS re
                    INNER JOIN eod_derivative AS der
                    ON re.Ticker=der.Ticker AND re.DateTime=der.DateTime
                    WHERE re.Ticker='{Ticker}' AND re.DateTime>='{min_dateX:%Y-%m-%d}'""",engine_public)
    if not data_eod.empty:
        ...
        fig=draw_oi(data,vn30,Ticker)
        return fig
    else:
        st.write("<b>BẠN ĐÃ NHẬP SAI MÃ VUI LÒNG NHẬP LẠI</b>",unsafe_allow_html=True)
        return None